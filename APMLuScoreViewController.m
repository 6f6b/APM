...
@interface APMLuScoreViewController ()
...
@implementation APMLuScoreViewController
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [APMHelper APMPOST:URL_GET_PAN_SCORE parameters:para progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if(state){
            ...
            if(arr.count>1){
                ...
                self.timeLabel.text = [NSString stringWithFormat:@"%.2fs",time];
                self.scoreLabel.text = [NSString stringWithFormat:@"%.2f/min",score];
                NSString *info = [NSString stringWithFormat:@"你当前的最快滑动速度超过了全国%d名玩家中的%.3f%@的玩家！位列第%d名！快去挑战自己的极限吧！",total,percent,@"%",rank];
                self.infoLabel.text = info;
